    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 1) {
            ...
            if (TeamHelper.checkTeamExistence(teamName)) {
                HashMap<String, Integer> teamTop = TeamHelper.getTeamTop();

                if (teamTop != null && !teamTop.isEmpty()) {
                    List<String> top = new ArrayList<>(teamTop.keySet());

                    top.forEach(t -> {
                        if (t.equals(teamName))
                            MessageSender.sendMessage(sender, teamName + " is ranked " + (top.indexOf(t) + 1) + " with an economy of " + teamTop.get(t));
                    });
                }
            } else {
